...
class ReasonerService:
    ...
    def _run_detectors(
        ...
        # Runtime contract: same-cycle signal visibility.
        # Signals generated by detector N in this cycle are NOT visible to
        # detector N+1 in the SAME cycle. They become visible in the NEXT cycle
        # when they are persisted to EvidenceStore via _append_reasoning_entry.
        # This is intentional and architecturally stable (ADR-046, ADR-052).
    ...
    def _propagate_evidence(
        ...
        for sig in new_signals:
            try:
                ...
                _logger.warning(
                    "evidence_store_capacity_exceeded | "
                    "q_idx=%d store_size=%d dropped_signals=%d",
                    question_index,
                    len(store.signals),
                    len(new_signals) - (len(store.signals) - len(memory.evidence_store.signals)),
                )
                break
        ...
        updated_metrics = prev_metrics.model_copy(update={
            "questions_answered": prev_metrics.questions_answered + 1,
            "total_evidence_signals": len(store.signals),
            "positive_evidence_count": len(
                [s for s in new_signals if s.polarity == EvidencePolarity.POSITIVE]
            ),
            "negative_evidence_count": len(
                [s for s in new_signals if s.polarity == EvidencePolarity.NEGATIVE]
            ),
            "last_reasoning_at_question_index": question_index,
        })
        ...
        return InterviewMemory(
            ...
            session_metrics=updated_metrics,
    ...
    def _compute_confidence(
        ...
        # Read from updated_memory so the incremented questions_answered is used.
        q_answered = updated_memory.session_metrics.questions_answered
