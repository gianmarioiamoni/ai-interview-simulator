# services/question_intelligence/pipelines/sql_pipeline_retrieval.py
...
class SqlPipelineRetrievalHelper:
    ...
    """SQL-only retrieval: merge relaxation stages until the candidate pool is deep enough."""
    ...
    def retrieve_candidates(
        ...
        for stage_filters in filter_stages:
            ...
            if len(merged_pool) >= _SQL_MIN_CANDIDATE_POOL:
                break
...
def retrieve_sql_candidates(
    ...
    helper = (
        sql_retrieval_helper
        if sql_retrieval_helper is not None
        else SqlPipelineRetrievalHelper()
    )
