...
class TestFinalReportDTOSeniority:
    ...
    def test_seniority_level_sourced_from_report(self):
        """Phase 9: seniority_level comes from report.seniority, not state."""
        state = build_state_with_execution(passed_tests=2, total_tests=2)
        report = make_report()
        state = state.model_copy(update={"report": report, "seniority_level": "junior"})
