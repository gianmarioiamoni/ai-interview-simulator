...
def test_grd_001_empty_fails_min_length() -> None:
    ...
    assert any("G-R1:min_length" in r for r in result.failed_rules)
...
def test_grd_003_exactly_min_length_passes_structural() -> None:
    ...
    assert not any("G-R1:min_length" in r for r in result.failed_rules)
...
def test_max_length_fails_above_2000() -> None:
    ...
    result = _validate(follow_up=text)
    assert any("G-R1b:max_length" in r for r in result.failed_rules)


def test_max_length_passes_at_2000() -> None:
    text = ("Redis technical eviction knowledge? " * 56)[:1999] + "?"
    result = _validate(follow_up=text)
    assert not any("G-R1b:max_length" in r for r in result.failed_rules)
...
def test_grd_004_no_keyword_overlap_fails() -> None:
    ...
    assert any("G-R2:keyword_overlap" in r for r in result.failed_rules)
...
def test_grd_005_keyword_overlap_passes() -> None:
    ...
    assert not any("G-R2:keyword_overlap" in r for r in result.failed_rules)
...
def test_grd_006_stopwords_only_answer_no_qualify() -> None:
    ...
    assert any("G-R2:no_qualifying_keywords_in_answer" in w for w in result.warnings)
    assert not any("G-R2:keyword_overlap" in r for r in result.failed_rules)
...
def test_grd_008_wrong_area_fails() -> None:
    ...
    assert any("G-R3:area_anchor" in r for r in result.failed_rules)
...
def test_grd_009_area_token_present_passes() -> None:
    ...
    assert not any("G-R3:area_anchor" in r for r in result.failed_rules)


def test_area_with_short_tokens_only_emits_warning() -> None:
    result = _validate(area="hr")  # token "hr" < 4 chars
    assert any("G-R3:no_qualifying_area_tokens" in w for w in result.warnings)
    assert not any("G-R3:area_anchor" in r for r in result.failed_rules)
...
def test_grd_010_verbatim_duplicate_fails() -> None:
    ...
    assert any("G-R4:not_duplicate" in r for r in result.failed_rules)
...
def test_grd_012_sufficiently_different_passes() -> None:
    ...
    assert not any("G-R4:not_duplicate" in r for r in result.failed_rules)
...
def test_grd_013_json_payload_fails() -> None:
    ...
    assert any("G-R5:not_json" in r for r in result.failed_rules)
...
def test_grd_015_plain_text_passes_json_rule() -> None:
    result = _validate()
    assert not any("G-R5:not_json" in r for r in result.failed_rules)
...
def test_grd_016_markdown_header_fails() -> None:
    ...
    assert any("G-R6:not_markdown" in r for r in result.failed_rules)
...
def test_grd_017_code_fence_fails_on_markdown() -> None:
    ...
    assert any(
        "G-R6:not_markdown" in r or "G-R9:no_code_block" in r
...
def test_grd_019_plain_text_passes_markdown_rule() -> None:
    result = _validate()
    assert not any("G-R6:not_markdown" in r for r in result.failed_rules)
...
def test_grd_020_unrendered_placeholder_fails() -> None:
    ...
    assert any("G-R7:no_placeholder" in r for r in result.failed_rules)
...
def test_grd_022_no_placeholder_passes() -> None:
    result = _validate()
    assert not any("G-R7:no_placeholder" in r for r in result.failed_rules)
...
def test_grd_023_no_question_mark_fails() -> None:
    ...
    assert any("G-R8:has_question_mark" in r for r in result.failed_rules)


def test_grd_024_question_mark_passes() -> None:
    result = _validate()
    assert not any("G-R8:has_question_mark" in r for r in result.failed_rules)
...
def test_code_block_triple_backtick_fails() -> None:
    ...
    assert any("G-R9:no_code_block" in r for r in result.failed_rules)
...
def test_html_tag_fails() -> None:
    ...
    assert any("G-R10:no_html_xml" in r for r in result.failed_rules)
...
def test_plain_text_passes_html_rule() -> None:
    result = _validate()
    assert not any("G-R10:no_html_xml" in r for r in result.failed_rules)
...
def test_template_text_your_question_here_fails() -> None:
    ...
    assert any("G-R16:no_template_text" in r for r in result.failed_rules)
...
def test_inj_sql_payload_fails() -> None:
    ...
    assert any("G-R14:no_sql_payload" in r for r in result.failed_rules)
...
def test_inj_python_payload_fails() -> None:
    ...
    assert any("G-R15:no_python_payload" in r for r in result.failed_rules)
