...
class UIResponse:
    ...
    setup_interactive: bool = True  # 🔥 NEW (lock instead of hide)
    ...
    def to_gradio_outputs(self) -> List[Any]:
        ...
        return [
            ...
            self.state,
            # -------------------------------------------------
            # SETUP INPUTS (VISIBLE + LOCKED)
            # -------------------------------------------------
            gr.update(visible=self.setup_visible, interactive=self.setup_interactive),
            gr.update(visible=self.setup_visible, interactive=self.setup_interactive),
            gr.update(visible=self.setup_visible, interactive=self.setup_interactive),
            gr.update(visible=self.setup_visible, interactive=self.setup_interactive),
            gr.update(
                visible=self.setup_visible,
                interactive=self.setup_interactive,
                value="Start Interview",  # 🔥 FIX BUTTON NONE
            ),
            # TITLE
